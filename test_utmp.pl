...
# man wtmp for details on what the various fields mean
...
my $format = "%9d %-13s %d %-5s %-20s\n";
my @fields = qw( pid type time line user );
...
#   logged             => 'in|out',
#   time today         => XXXX,
#   daily average time => XXXX,
#   month total time   => XXXX,
#   times              => [
#     { pid => PPPP, in => IIII, out => OOOO },
#   ],

# PID     USER_PROCESS username is login
# SAMEPID DEAD_PROCESS username is logout

for my $record ( getutx() ) {

  my %h; @h{ @fields } = @{ $record }{ map { "ut_$_" } @fields };
  $h{ type } = $type{ $h{ type } };

  if ( $h{ type } eq 'BOOT_TIME' ) {
